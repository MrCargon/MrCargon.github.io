/**
 * tictactoe-loader.js - Dual-Boot Module Entry Point
...
async function waitForCSS(selector, timeoutMs = 5000) {
    ...
    if (!cssLink) {
        console.warn('[TicTacToe:Loader] CSS link not found:', selector);
    ...
    return new Promise((resolve) => {
        const timeout = setTimeout(() => {
            console.warn('[TicTacToe:Loader] CSS load timeout, continuing anyway');
            resolve(false);
        }, timeoutMs);
        ...
        cssLink.addEventListener('error', () => {
            clearTimeout(timeout);
            console.error('[TicTacToe:Loader] CSS failed to load');
            resolve(false);
        });
    });
}
...
async function initStandalone() {
    console.log('[TicTacToe:Loader] Standalone mode detected - auto-booting');

    const container = document.getElementById('game-container');
    if (!container) {
        console.error('[TicTacToe:Loader] #game-container not found');
        return false;
    }

    // Wait for CSS before init
    await waitForCSS('link[href*="TicTacToe.css"]');

    // Clean up existing instance if present
    if (gameInstance) {
        console.warn('[TicTacToe:Loader] Existing instance detected - cleaning up');
        try {
            gameInstance.destroy();
        } catch (e) {
            console.error('[TicTacToe:Loader] Cleanup failed:', e);
        }
        gameInstance = null;
    }
    ...
    try {
        gameInstance = new TicTacToe(container);
        ...
        setupExitButton();
        console.log('[TicTacToe:Loader] Game initialized successfully');
        return true;

    } catch (error) {
        console.error('[TicTacToe:Loader] Fatal error:', error);
...
function initializeGame() {
    if (typeof window === 'undefined') {
        console.error('[TicTacToe:Loader] Window object not available');
        return false;
    }
    ...
    if (isStandalone) {
        ...
        console.log('[TicTacToe:Loader] Portfolio mode - exporting to window.TicTacToe');
