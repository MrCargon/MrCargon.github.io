...
class ProjectsPageManager {
 ...
 // Cleanup active game
        if (this.activeGame && this.gameInstances.has(this.activeGame)) {
            ...
            // Note: GameScaler cleanup moved to PageManager.js
            // where game launch/close actually happens

            if (game && typeof game.destroy === 'function') {
                game.destroy();
            } else if (game && typeof game.cleanup === 'function') {
                game.cleanup();
            }
 ...
 // CSS handles rotation, but we can update for screen readers
    ...
    /**
     * Golden Rules assertion helper - Rule 5 (compliance * Side) -effect free assertion for defensive programming
     */
    c_assert(condition) {
