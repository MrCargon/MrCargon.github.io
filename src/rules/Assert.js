...
class Assert {
 ...
 // Rule 5: Self-validation
        const conditionValue = (typeof condition !== 'boolean' && !condition)
            ? Boolean(condition)
            : condition;
 ...
 // Rule 5: Validate parameter name
        const paramName = (typeof parameterName !== 'string')
            ? 'unknown_parameter'
            : parameterName;
        ...
        if (isNullish) {
            ...
            const message = `${paramName} must not be ${nullType}`;
            return this.assert(false, message);
        }

        return this.assert(true, `Non-null validation passed for ${paramName}`);
    }
 ...
 // Rule 5: Parameter validation
        const funcName = (typeof functionName !== 'string')
            ? 'unknown_function'
            : functionName;
        ...
        if (!condition) {
            const message = `Precondition failed in ${funcName}: ${desc}`;
            return this.assert(false, message);
        }

        return this.assert(true, `Precondition satisfied in ${funcName}: ${desc}`);
    }
 ...
 // Rule 5: Parameter validation
        ...
        if (!condition) {
            const message = `Postcondition failed in ${funcName}: ${desc}`;
            return this.assert(false, message);
        }

        return this.assert(true, `Postcondition satisfied in ${funcName}: ${desc}`);
    }
