...
class Lenia {
    ...
    /**
     * Paint living matter into the CURRENT state without reseeding.
     *
     * Same GPU-stamp trick as ConwayLife.paint: rendering a small quad into the live
     * target with autoClear off, rather than reading 256x256 floats back to the CPU on
     * every pointer move. Lenia's state is continuous, so this deposits a solid patch that
     * the growth function then either grows into an organism or dissolves — which is the
     * honest behaviour, not a guarantee of life.
     * Rule 5: 2 asserts.
     */
    paint(cx, cy, alive, radius) {
        ...
        const n = this.size;
        const r = Math.max(1, Math.min(64, radius | 0));      // Rule 2: bounded brush
        const span = (2 * r + 1) / n;
        ...
        this._stamp.scale.set(span * 2, span * 2, 1);
